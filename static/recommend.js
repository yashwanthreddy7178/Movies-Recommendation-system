...
var myAPI = '5b3d643a469d23715d825382710a7936'  // global string to be consistent with future usages elsewhere
$(function() {
...
function markAsWatched(movieId) {
  // Assuming you have a function to remove the watched movie from the list and add a new recommendation
  // Replace the following line with your actual logic
  console.log(`Movie ${movieId} marked as watched. Add logic to remove and add new recommendation.`);
}
...
function get_movie_cast(movie_id,my_api_key){
  ...
  function markAsWatched(movieId) {
    // Send an AJAX request to the server to indicate the movie is watched
    fetch('/mark-watched', {
      method: 'POST',
      headers: {
    ...
    .then(data => {
      // Update the recommended movies list based on the server response
      const watchedMovieCard = document.querySelector(`.movie-card[data-movie-id="${movieId}"]`);
      watchedMovieCard.remove();
      // Add new movie data to the list
      const newMovieCard = createMovieCard(data.newMovie);
      recommendationsContainer.appendChild(newMovieCard);
    })
    .catch(error => {
